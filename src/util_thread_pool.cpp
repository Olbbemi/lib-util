...
bool thread_pool_c::create_pool(const std::string& identification, uint16_t max_cnt)
{
	...
	for(auto& consumer_future : check_consumer_ready)
	{
		...
		{
			U_LOG_ROTATE_FILE(util::LOG_LEVEL::ERROR, "partial consumer thread isn't ready.");
			return false;
		}
	}

	U_LOG_ROTATE_FILE(util::LOG_LEVEL::DEBUG, "all consumer thread is ready. count:{}", _max_cnt);
	return true;
}
	...
	{
		{
			...
			if(true == shutdown)
			{
				U_LOG_ROTATE_FILE(util::LOG_LEVEL::DEBUG, "consumer thread_id({}) is shutdown.", _id);
				break;
			}
			...
			{
				U_LOG_ROTATE_FILE(util::LOG_LEVEL::WARNING, "consumer thread_id({}) is wake up, but task_queue is empty.", _id);
				continue;
			}
...
void thread_pool_c::_task_producer_thread()
{
	while(true)
	{
		...
		if(true == shutdown)
		{
			U_LOG_ROTATE_FILE(util::LOG_LEVEL::DEBUG, "producer thread is shutdown.");
			break;
		}
		...
		while(true)
		{
			...
			if(Green.min <= wait_cnt && wait_cnt <= Green.max) {
				...
				U_LOG_ROTATE_FILE(util::LOG_LEVEL::CRITICAL, "wait_cnt({}) is over the Red limit({}).", wait_cnt, Red.max);
			}
